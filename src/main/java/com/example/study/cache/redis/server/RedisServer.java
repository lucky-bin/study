package com.example.study.cache.redis.server;
...
public class RedisServer {
    ...
    @CachePut(cacheManager = "cacheManager", value = "cache-1", key = "#key", condition = "#result ne null")
    ...
    @CacheEvict(cacheManager = "cacheManager", key = "#key")
    ...
    public void list() {
        ...
        // 消费者线程简例
        while (true) {
            String item =  redisTemplate.opsForList().leftPop("queue_1");
            if (item == null) break;
    ...
    public void set() {
        // 取出两个人共同关注的好友
        // 每个人维护一个set
        redisTemplate.opsForSet().add("user_zhangsan", "userC", "userD", "userE");
        redisTemplate.opsForSet().add("user_lisi", "userC", "userE", "userF");
        // 取出共同关注
        Set<String> sinter = redisTemplate.opsForSet().intersect("user_hash", "user_peak");
        System.out.println("共同关注了zhangsan、lisi的用户："+sinter);
